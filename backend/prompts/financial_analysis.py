...
def build_financial_analysis_prompt(dupont: dict, anomalies: list, company_name: str, report_date: str) -> str:
    ...
    at_val = dupont['asset_turnover']
    at_display = f"{at_val:.4f}" if at_val and at_val > 0 else "N/A（总资产数据缺失）"
    nm_val = dupont['net_margin']
    nm_display = f"{nm_val:.4f}" if nm_val and nm_val > 0 else "N/A"
    roe_display = f"{dupont['roe']:.4f}" if dupont['roe'] and dupont['roe'] > 0 else "N/A"

    dupont_str = f"""杜邦分解结果:
- ROE: {roe_display}
- 净利率: {nm_display}
- 资产周转率: {at_display}
- 权益乘数: {dupont['equity_multiplier']:.4f}
